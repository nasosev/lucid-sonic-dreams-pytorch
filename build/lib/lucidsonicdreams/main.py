...
class LucidSonicDream:
    ...
    for i, (noise_batch, class_batch) in enumerate(tqdm(dl, position=0, desc="Generating frames")):
        ...
        # Otherwise, generate frames with StyleGAN(2)
            ...
            if self.use_tf:
               ...
               # PyTorch branch for generating frames
                ...
                # Debug: print the raw output shape from synthesis
                print("Raw synthesized image_batch shape:", image_batch.shape)
                # Permute to get shape (batch, height, width, channels)
                image_batch = image_batch.permute(0, 2, 3, 1) * 127.5 + 128
                image_batch = image_batch.clamp(0, 255).to(torch.uint8)
                # Only remove batch dimension if it is of size 1 AND the remaining tensor has 3 dimensions
                if image_batch.ndim == 4 and image_batch.shape[0] == 1:
                    image_batch = image_batch.squeeze(0)
                print("Final image_batch shape after squeeze:", image_batch.shape)
                image_batch = image_batch.cpu().numpy()

                # Check and fix the image shape if the width dimension is missing.
                if image_batch.ndim == 2:
                    # If we get a 2D array of shape (height, channels) (e.g. (512, 3)),
                    # assume the width dimension is missing. We expect width = self.input_shape.
                    h, c = image_batch.shape
                    if c == 3:
                        print(f"Warning: generated image is 2D with shape {image_batch.shape}; "
                              "treating it as missing width and replicating columns.")
                        image_batch = np.tile(image_batch[:, np.newaxis, :], (1, self.input_shape, 1))
                elif image_batch.ndim == 3:
                    h, w, c = image_batch.shape
                    if w == 1:
                        print("Warning: generated image width is 1; replicating columns to match expected width.")
                        image_batch = np.repeat(image_batch, repeats=self.input_shape, axis=1)
                    elif w != self.input_shape:
  ...
  def store_imgs(self, file_names, final_images, resolution):
      for file_name, final_image in tqdm(zip(file_names, final_images), position=1, leave=False, desc="Storing frames", total=len(file_names)):
          ...
          print(f"Storing frame {file_name}: shape {arr.shape}, dtype {arr.dtype}")

          # If the image appears 2D (height, channels) instead of (height, width, channels)
          if arr.ndim == 2 and arr.shape[1] == 3:
              print(f"Warning: image shape {arr.shape} is 2D; replicating columns to match expected width {self.input_shape}.")
              # Insert a new axis for width and replicate it self.input_shape times
              arr = np.tile(arr[:, np.newaxis, :], (1, self.input_shape, 1))
          # Alternatively, if the image is 3D but the width is incorrect
          elif arr.ndim == 3:
              h, w, c = arr.shape
              if w != self.input_shape:
                  print(f"Warning: image width {w} does not match expected {self.input_shape}; replicating columns.")
                  # Calculate repeat factor (ensure it's an integer)
                  repeat_factor = int(np.ceil(self.input_shape / w))
                  arr = np.repeat(arr, repeats=repeat_factor, axis=1)
                  # Now trim to exactly self.input_shape columns
                  arr = arr[:, :self.input_shape, :]

          if arr.size == 0:
              print(f"Frame {file_name} is empty. Skipping...")
              continue
          try:
              pil_img = Image.fromarray(arr, mode='RGB')
          except Exception as e:
              print(f"Error creating image for frame {file_name}: {e}")
              continue
          if resolution:
              pil_img = pil_img.resize((resolution, resolution))
          pil_img.save(os.path.join(self.frames_dir, file_name + '.jpg'), subsample=0, quality=95)
